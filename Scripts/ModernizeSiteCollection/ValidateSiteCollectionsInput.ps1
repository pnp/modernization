<#
.SYNOPSIS
Validates the CSV input file for the bulk "modernize site collections" script. 

.EXAMPLE
PS C:\> .\ValidateSiteCollectionsInput.ps1
#>
...
$date = Get-Date
$logfile = ((Get-Item -Path ".\" -Verbose).FullName + "\ModernizationInputValidation_log_" + $date.ToFileTime() + ".txt")
$global:strmWrtLog=[System.IO.StreamWriter]$logfile
$global:Errorfile = ((Get-Item -Path ".\" -Verbose).FullName + "\ModernizationInputValidation_error_" + $date.ToFileTime() + ".txt")
...
#region Read tenant themes
$tenantThemes = Get-PnPTenantTheme -Connection $tenantContext
#endregion
...
foreach($row in $csvRows)
{
    if($row.SiteUrl.Trim() -ne "")
    {
        ...
        LogWrite "[VALIDATING] $siteUrl with alias [$siteAlias], theme [$siteTenantTheme] and classification [$siteClassification]"
        ...
        try 
        {
            ...
            elseif( $null -eq ($tenantThemes | Where-Object {$_.Name -eq $siteTenantTheme}))
            {
                $message = "[ERROR] $siteUrl : Tenant theme [$siteTenantTheme] is not available in this tenant"
                LogWrite $message Red
                LogError $message
            }                     
            else 
